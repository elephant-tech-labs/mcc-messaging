...
function loadZohoSdk(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      ...
      existing.addEventListener("error", () => reject(new Error("Zoho widget SDK failed to load")), {
        once: true,
      });
      return;
    }
...
function getCurrentUser(data: unknown): ZohoUser | null {
...
export default function ContactMessagingWidget() {
  ...
  const fetchHistory = useCallback(
    async (showLoading: boolean) => {
      ...
      try {
        ...
        const changed = nextLastId !== lastMessageIdRef.current;
        ...
        if (firstLoadRef.current || changed) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        await loadZohoSdk();
        if (cancelled || !window.ZOHO) return;
        ...
        const userResponse = await window.ZOHO.CRM.CONFIG.getCurrentUser();
        if (!cancelled) setCurrentUser(getCurrentUser(userResponse));

        await Promise.resolve(window.ZOHO.embeddedApp.init());
        void window.ZOHO.CRM.UI?.Resize?.({ height: "600", width: "1000" });
      ...
      <section className={styles.chat} aria-label="MCC contact messaging">
        ...
        {error ? (
          <div className={styles.errorBanner} role="alert">
            <span>{error}</span>
            {contactId ? (
              <button type="button" onClick={() => void fetchHistory(true)}>
                Retry
              </button>
            ) : null}
          </div>
        ...
        <div className={styles.messages} aria-live="polite">
          ...
          {empty ? (
            <div className={styles.emptyState}>
              ...
              <p>Messages sent here will use the Contact&apos;s Phone field and stay attached to this CRM thread.</p>
            </div>
          ) : null}
                ...
                return (
                  ...
                  >
                    <div className={`${styles.bubble} ${outgoing ? styles.outgoingBubble : styles.incomingBubble}`}>
                      {outgoing && item.sent_by_name ? (
                        <div className={styles.sender}>{item.sent_by_name}</div>
                      ) : null}
                      <div className={styles.body}>{item.body || "(No text)"}</div>
                      <div className={styles.meta}>
                        <span>{formatMessageTime(item.created_at)}</span>
                        {outgoing ? (
                          <span className={state.failed ? styles.failedStatus : styles.deliveryStatus}>
                            {state.label}
                          </span>
                        ) : null}
                      </div>
